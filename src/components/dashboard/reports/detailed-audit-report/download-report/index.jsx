...
const DownloadDetailedAuditReport = () => {
    ...
    const { singleInternalAuditReport, loading: reportLoading } = useSelector(
        (state) => state.consolidationReport
    );

    // Prepare company logo preview (base64 image)
    const defaultLogoBase64 = user?.[0]?.company?.[0]?.logo?.fileData || "";
    const logoPreview = `data:image/jpeg;base64,${defaultLogoBase64}` || "";
        ...
        <div>
            {reportLoading ? (
                ...
                <>
                    ...
                    {/* PDF Download Button with informative loading state */}
                    <PDFDownloadLink
                        document={
                            <MyDocument
                            ...
                            loading ? (
                                <div className="d-flex flex-column align-items-start">
                                    ...
                                    <small className="text-muted mt-2">
                                        If the observations are large, the PDF file may take up to 3
                                        minutes to generate. During this time, your screen may
                                        appear frozen — please do not refresh the page. Once the{" "}
                                        <strong>Download PDF</strong> button becomes active
                                        (visible on hover), you will be able to download your PDF.
                                        <br />
                                        <strong>Note:</strong> The company logo must be added by an
                                        admin in order to generate the PDF correctly.
                                    </small>
                                </div>
                            ) : (
