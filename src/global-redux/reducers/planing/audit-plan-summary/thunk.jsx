...
export const getAllAuditPlanSummary = async (data, thunkAPI) => {
  try {
    ...
    let props = await axios.get(
      `${baseUrl}/auditPlanningAndScheduling/auditPlanSummary/getAllByCompanyId/light?companyId=${data?.companyId}&pageNo=${data?.page}&noOfRecords=${data?.itemsPerPage}&year=${data?.year}`,
      {
        headers: {
...
export const submitAuditPlanSummary = async (data, thunkAPI) => {
  try {
    const { user } = thunkAPI.getState().auth;
    let props = await axios.post(
      `${baseUrl}/auditPlanningAndScheduling/auditPlanSummary/update`,
