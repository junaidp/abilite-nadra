...
export const getAllJobPrioritization = async (data, thunkAPI) => {
  try {
    ...
    let props = await axios.get(
      `${baseUrl}/auditPlanningAndScheduling/jobPrioritization/getAllByCompanyId/light?companyId=${data?.companyId}&pageNo=${data?.page}&noOfRecords=${data?.itemsPerPage}`,
      {
        headers: {
...
export const submitJobPrioritization = async (data, thunkAPI) => {
  try {
    const { user } = thunkAPI.getState().auth;
    let props = await axios.post(
      `${baseUrl}/auditPlanningAndScheduling/jobPrioritization/update`,
